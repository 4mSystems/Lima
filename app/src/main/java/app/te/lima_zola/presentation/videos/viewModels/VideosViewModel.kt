...
import app.te.lima_zola.domain.videos_articles.use_case.VideosArticlesUseCase
...
class VideosViewModel @Inject constructor(
  private val videosArticlesUseCase: VideosArticlesUseCase,
  ...
  private fun getData(cat_id: Int) {
    viewModelScope.launch {
      ...
      val videosArticlesData =
        withContext(Dispatchers.IO) { async { videosArticlesUseCase.invoke(cat_id) } }
  ...
  fun getVideosArticles(cat_id: Int) {
    viewModelScope.launch {
      videosArticlesUseCase.invoke(cat_id).collect { result ->
        _videoArticlesResponse.value = result
      }
    }
  }
