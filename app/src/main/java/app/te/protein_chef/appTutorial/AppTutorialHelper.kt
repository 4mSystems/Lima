...
class AppTutorialHelper private constructor(builder: Builder) : LifecycleObserver {
  ...
  private var btnPrevious: MaterialButton =
    MaterialButton(ContextThemeWrapper(activity, R.style.skipButtonTheme))
  private var btnOpenApp: MaterialButton =
  ...
  private fun setUpNextButton() {
    btnNext = MaterialButton(ContextThemeWrapper(activity, R.style.nextButtonTheme))
    btnNext.apply {
      id = View.generateViewId()
  ...
  private fun setUpConstraintSet() {
    ...
    constraintSet.constrainHeight(btnNext.id, 0)
    ...
    constraintSet.connect(btnNext.id, ConstraintSet.END, btnPrevious.id, ConstraintSet.START, 30)
    //btnPrevious
    constraintSet.constrainWidth(btnPrevious.id, 0)
    ...
    constraintSet.connect(btnPrevious.id, ConstraintSet.START, btnNext.id, ConstraintSet.END)
  ...
  class Builder(internal var activity: Activity, internal var lifecycle: Lifecycle) {
    ...
    @ColorRes
    internal var btnPreviousTextColor: Int = R.color.white
